  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="h-full flex flex-col">
        {/* Enhanced Header */}
        <motion.header
          className="sticky top-0 z-50 bg-white border-b border-border/50"
          initial={{ y: -100 }}
          animate={{ y: 0 }}
                  ...
                  <div className="min-w-[160px] sm:min-w-[200px] text-center">
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="ghost" className="hover:bg-primary/5 px-4 h-auto py-1">
                          <motion.h2
                            key={getDateRange()}
                            className="text-lg sm:text-xl font-semibold text-foreground cursor-pointer flex items-center gap-2"
                        ...
                        <div className="space-y-2">
                          {Object.entries(EVENT_TYPE_CONFIG).map(([key, config]) => (
                            <div key={key} className="flex items-center space-x-2">
                              <Checkbox
                                id={`type-${key}`}
                                checked={filter.event_types?.includes(key) || false}
                                onCheckedChange={(checked) => {
                                  const currentTypes = filter.event_types || [];
                                  const newTypes = checked
                                    ? [...currentTypes, key]
                                    : currentTypes.filter(t => t !== key);
                                  onFilterChange({ ...filter, event_types: newTypes.length > 0 ? newTypes : undefined });
                                }}
                              />
                              <Label htmlFor={`type-${key}`} className="flex items-center space-x-2 cursor-pointer">
                            ...
                            <div key={priority} className="flex items-center space-x-2">
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  ...
                                  onFilterChange({ ...filter, priorities: newPriorities.length > 0 ? newPriorities : undefined });
                                }}
                              />
                              <Label htmlFor={`priority-${priority}`} className="capitalize cursor-pointer">
                                {priority}
                              </Label>
                            </div>
                          ))}
