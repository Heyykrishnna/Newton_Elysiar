...
export const WeekView: React.FC<WeekViewProps> = ({
  ...
  useEffect(() => {
    if (scrollRef.current) {
      ...
      const scrollPosition = Math.max(0, currentHour * 128 - 200); // 128px per hour (64px * 2 slots)
  ...
  const getEventPosition = (event: CalendarEvent, dayWidth: number) => {
    ...
    const height = Math.max((duration / 30) * 64, 32); // Minimum 32px height
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="border-b border-border/50 bg-white shadow-sm">
        <div className="grid grid-cols-8 gap-0">
          {/* Time Column Header */}
          <div className="p-5 border-r border-border/50 bg-muted/20">
          ...
          {weekDays.map((day, index) => {
            ...
            return (
              <motion.div
                key={day.toISOString()}
                className={cn(
                  "p-5 text-center border-r border-border/50 last:border-r-0 transition-all duration-200",
                  isDayToday && "bg-gradient-to-br from-primary/10 to-primary/5"
                )}
              ...
              >
                <div className="space-y-2">
                  ...
                  <motion.div
                    className={cn(
                      "text-lg font-bold mx-auto w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200",
                      isDayToday
                        ? "bg-gradient-to-br from-primary to-primary/80 text-primary-foreground shadow-lg shadow-primary/30"
                        : "text-foreground hover:bg-accent/30 hover:shadow-md"
                    )}
                ...
                <div className="mt-3 space-y-1.5">
                  {eventsByDay[format(day, 'yyyy-MM-dd')]
                    ...
                    .map((event, eventIndex) => (
                      <motion.div
                        key={event.id}
                        className="px-2.5 py-1.5 rounded-lg cursor-pointer text-xs font-semibold truncate hover:scale-105 transition-all duration-200 shadow-sm"
                        style={{
      ...
      <div className="flex-1 overflow-hidden">
        <ScrollArea className="h-full" ref={scrollRef}>
          <div className="relative">
            ...
            <div className="grid grid-cols-8 gap-0">
              {/* Time Labels Column */}
              <div className="border-r border-border/50 bg-white">
                {timeSlots.map((slot, index) => (
                  <div
                    key={slot.time}
                    className={cn(
                      "h-16 px-4 py-3 border-b border-border/30 text-xs text-muted-foreground font-semibold",
                    ...
                    {timeSlots.map((slot, slotIndex) => (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                          <div className="text-xs text-muted-foreground bg-background/90 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-md border border-border/50 font-medium">
                            + Add event
                          </div>
                        </div>
                      </motion.div>
