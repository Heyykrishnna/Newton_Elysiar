...
export default function MyLibrary() {
  ...
  const [selectedBookForReviews, setSelectedBookForReviews] = useState<any>(null);
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {activeIssues.map((issue: any) => {
                    ...
                    return (
                      <Card key={issue.id} className={`overflow-hidden hover:shadow-xl transition-shadow border-2 ${isOverdue ? 'border-red-300' : isDueSoon ? 'border-yellow-300' : 'border-blue-300'
                        }`}>
                        ...
                        <CardContent className="space-y-3">
                          ...
                          <div className={`p-3 rounded-lg ${isOverdue ? 'bg-red-100 dark:bg-red-900/20 text-red-900 dark:text-red-300' :
                          ...
                          {/* Rate & Review Button - Only for returned books */}
                          {issue.status === 'returned' && (
                            <div className="flex gap-2 mt-4">
                              <BookRatingDialog
                                bookId={issue.book_id}
                                bookTitle={issue.books?.title || ''}
                                trigger={
                              ...
                              <Dialog>
                                <DialogTrigger asChild>
                                  <Button
                                    ...
                                    size="sm"
                                    className="flex-1"
                                    onClick={() => setSelectedBookForReviews(issue.books)}
                                  >
                                ...
                                <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                                  <DialogHeader>
                                    <DialogTitle>Book Reviews</DialogTitle>
                                  </DialogHeader>
                                  {selectedBookForReviews && (
                                    <BookReviewsList bookId={issue.book_id} />
                                  )}
                                </DialogContent>
                              </Dialog>
                            </div>
