...
import { ProctorApp, ProctoredContextApp } from '@newtonschool/react_proctoring_library';
...
const TakeTest = () => {
  ...
  const [topHoverStrikes, setTopHoverStrikes] = useState(0);
  const [generalStrikes, setGeneralStrikes] = useState(0);
  ...
  // Ref to access latest submit handler in event listeners
  const handleSubmitRef = React.useRef(handleSubmitTest);
  useEffect(() => {
    handleSubmitRef.current = handleSubmitTest;
  }, [handleSubmitTest]);

  // Monitor fullscreen state and violations
  useEffect(() => {
    const handleFullscreenChange = () => {
      const isFull = !!document.fullscreenElement;
      setIsFullscreen(isFull);
      
      if (testStarted && !isFull) {
        setGeneralStrikes(prev => {
           const newStrikes = prev + 1;
           if (newStrikes >= 3) {
             toast({
               title: "Test Terminated",
               description: "Multiple fullscreen violations detected (Code: 302). Auto-submitting...",
               variant: "destructive",
               duration: 10000
             });
             ...
             toast({
               ...
               description: `Fullscreen exit detected! Warning ${newStrikes}/3. Do not exit fullscreen.`,
               variant: "destructive",
               duration: 5000
             });
           }
      ...
      if (testStarted && document.hidden) {
        setGeneralStrikes(prev => {
           ...
           if (newStrikes >= 3) {
             toast({
               title: "Test Terminated",
               description: "Multiple background violations detected (Code: 303). Auto-submitting...",
               variant: "destructive",
               duration: 10000
             });
             handleSubmitRef.current();
             ...
             toast({
               title: "Violation Warning",
               description: `Tab switch/minimize detected! Warning ${newStrikes}/3.`,
               variant: "destructive",
               duration: 5000
             });
           }
  ...
  // Strict Mode Monitor: Warn user when mouse approaches top system bar (Mac specific concern)
  useEffect(() => {
    if (!testStarted) return;

    const handleMouseMove = (e: MouseEvent) => {
      // Mac menu bar is usually top 20-30px. We warn if < 50px
      if (e.clientY < 50) {
        if (!isNearTopRef.current) {
          isNearTopRef.current = true;
          setIsMouseNearTop(true);
          
          setTopHoverStrikes(prev => {
            const newStrikes = prev + 1;
            if (newStrikes >= 3) {
              toast({
                title: "Test Terminated",
                description: "Multiple top-screen violations detected (Code: 301). Auto-submitting...",
                variant: "destructive",
                duration: 10000
              });
              handleSubmitRef.current();
              ...
              toast({
                title: "Violation Warning",
                description: `Top screen interaction restricted! Warning ${newStrikes}/3.`,
                variant: "destructive",
                duration: 4000
              });
            }
        ...
        if (isNearTopRef.current && e.clientY >= 50) {
    ...
    const handleClick = (e: MouseEvent) => {
      if (e.clientY < 50) {
        toast({
          ...
          description: "Clicking here risks test termination. Move mouse down immediately.",
          variant: "destructive",
          duration: 3000
        });
      }
  ...
  // Strict mode effects - cleaned up to rely on proctoring library where possible
  useEffect(() => {
    if (testStarted) {
      // Prevent page refresh and navigation (still useful to keep manual prevention for accidental unloads)
  ...
  // Logger for proctoring events
  const handleProctorLog = (data: any) => {
    console.log('Proctoring Event:', data);
    // You can handle violations here if the library exposes them via logger
  };
              ...
              <CardContent className="space-y-6">
                <div className="text-center space-y-4">
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    <br/>
                    <strong>Note:</strong> This test is proctored. Fullscreen mode will be enforced.
                  </div>
                </div>
                <div className="flex space-x-4">
                  <Button
  ...
  return (
    <ProtectedRoute>
      <ProctoredContextApp
        contextProvider={ProctorContext}
        config={{
          proctorParams: {
            fullscreenExit: true,
            ...
            video: false,
            openHeimdall: false,
            downloadHeimdall: false
          }
        }}
      >
        <ProctorApp
          proctoringIdentifier={testId || 'unknown-test'}
          proctoredContext={ProctorContext}
          logger={handleProctorLog}
          customPermissionView={TestPermissions}
          config={{
            proctorParams: {
              fullscreenExit: true,
              copyPasteProctoring: true,
        ...
        >
          <div className="fixed inset-0 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden">
            {/* Strict Mode Overlay Warning for Mac/System UI */}
            {isMouseNearTop && (
               <div className="fixed top-0 left-0 w-full h-24 z-[100] flex justify-center pt-4 pointer-events-none bg-gradient-to-b from-red-600/40 to-transparent backdrop-blur-[2px]">
                  <div className="bg-red-600 text-white px-6 py-3 rounded-full text-base font-bold shadow-2xl flex items-center gap-2 animate-bounce border-2 border-white/20">
                    <AlertTriangle className="w-5 h-5" />
                    WARNING: MOVE MOUSE AWAY FROM TOP EDGE!
                  </div>
               </div>
            )}
            
            {/* Strict Mode Warning */}
            {isFullscreen && !isMouseNearTop && (
