...
Format as JSON array with keys: front, back`;
      ...
      const { code, language, files, analyzeExecution } = await req.json();
      
      systemPrompt = `You are an expert supportive coding mentor. Your goal is to analyze code and provide constructive, encouraging, and specific feedback.
Focus on:
1. **Correctness**: Does the code do what it looks like it intends to do?
2. **Best Practices**: Are there better ways to write this in the specific language?
3. **Efficiency**: Time/Space complexity improvements.
4. **Style**: Readability and conventions.
5. **Structure**: (For multi-file projects) How well is the project organized?

Output Format: JSON object with keys:
- strengths: string[] (What was done well, 3-5 items)
- improvements: string[] (Specific things to fix or improve, 3-5 items)
- suggestions: string (A supportive summary paragraph with actionable advice)
- complexity: { time: string, space: string } (Big O notation)
- correctedCode: string (Optional, improved version if significant fixes are needed)
`;

      if (analyzeExecution) {
        systemPrompt += `\n\nALSO analyze the execution flow:
- Describe step-by-step what happens when the code runs
- Track variable values at each step
- Identify potential bugs or edge cases
- Explain the logic flow clearly for learning purposes`;
      }
      ...
      if (files && typeof files === 'object') {
         ...
         userPrompt = `Analyze this project structure and code:\n\n${fileContext}`;
      } else {
